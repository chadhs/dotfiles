...
Startup GC / file-name-handler tuning lives in =early-init.el= and is restored at the end of this file after packages load.
...
=auto-dark= follows the OS appearance. =,ct= manually toggles between Solarized dark/light without the unmaintained =cycle-themes= package (and its deprecated =cl= dependency).

#+BEGIN_SRC emacs-lisp
  (defun cs/toggle-solarized-theme ()
    "Toggle between solarized-dark and solarized-light."
    (interactive)
  ...
  (when (display-graphic-p)
    (evil-leader/set-key "ct" #'cs/toggle-solarized-theme))
...
Disabled: global emojify caused massive performance degradation. Keep the stanza commented if you ever want to re-enable selectively.
...
Tree-sitter grammar sources. Install missing grammars on demand with =M-x cs/treesit-install-grammars= (not during every startup).
...
#+BEGIN_SRC emacs-lisp
  (use-package treesit
    ...
    (defvar cs/treesit-languages
      '(javascript typescript tsx json css html regex ruby python go java)
      "Grammars managed by `cs/treesit-install-grammars'.")

    (defun cs/treesit-install-grammars ()
      "Install any missing tree-sitter grammars from `cs/treesit-languages'."
      (interactive)
      (dolist (lang cs/treesit-languages)
...
Alchemist is largely unmaintained; keep syntax highlighting via =elixir-mode=. Prefer ElixirLS through =lsp-mode= if you return to Elixir work.
...
#+BEGIN_SRC emacs-lisp
  ;; Restore startup GC / file-name-handler settings from early-init.el
  (setq gc-cons-threshold (* 64 1024 1024) ; 64MB while editing
        gc-cons-percentage 0.1)
  (when (boundp 'cs--file-name-handler-alist)
    (setq file-name-handler-alist cs--file-name-handler-alist))
