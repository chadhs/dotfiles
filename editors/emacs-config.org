...
*** Trust corporate TLS inspection

Optional: if =~/.config/certs/emacs-ca-bundle.pem= exists (work Mac), trust it so HTTPS package archives work. Missing on personal machines is fine — this is a no-op then.

Regenerate after corporate cert rotation (example):
=cat /etc/ssl/cert.pem ~/.config/certs/zscaler-root-ca.pem > ~/.config/certs/emacs-ca-bundle.pem=

#+BEGIN_SRC emacs-lisp
  (let ((ca-bundle (expand-file-name "~/.config/certs/emacs-ca-bundle.pem")))
    (when (file-readable-p ca-bundle)
      (setq gnutls-trustfiles (list ca-bundle)
            gnutls-verify-error t)))
#+END_SRC

*** Define package repositories

Use HTTPS for all archives. On work Macs the optional CA bundle above
covers corporate TLS inspection; on personal Macs the system trust store
is enough.
...
Quelpa is only needed for source installs (e.g. commented Copilot setup). Keep it deferred so normal startups do not pay for it.
...
:PROPERTIES:
:CUSTOM_ID: general-package-configuration
:END:
...
#+BEGIN_SRC emacs-lisp
  (use-package evil-leader
    ...
    ;; Emacs 31 visits files as fundamental-mode then the real major
    ;; mode. evil-leader only binds "," when it turns on, so re-enable
    ;; after major-mode is set so mode-local maps attach.
    (defun cs/reenable-evil-leader-mode ()
      "Turn evil-leader off and on so the mode-local map matches `major-mode'."
      (when (bound-and-true-p evil-local-mode)
        (evil-leader-mode -1)
        (evil-leader-mode 1)))
    (add-hook 'after-change-major-mode-hook #'cs/reenable-evil-leader-mode t))
...
Solarized is used in graphical, terminal, and daemon sessions. See
[[file:THEMES.md][editor theme notes]] for troubleshooting and restoring Omarchy palette following.
...
*** auto theme (OS light / dark)

=auto-dark= follows the OS appearance, using =osascript= on macOS and
automatic backend detection elsewhere, including D-Bus on Linux. Manual
theme cycling stays disabled so it cannot fight OS auto-toggling.
...
#+BEGIN_SRC emacs-lisp
  (use-package auto-dark
    ...
    (auto-dark-detection-method (when (eq system-type 'darwin) 'osascript))
    :config
    (auto-dark-mode 1))
#+END_SRC
...
Dedicated zsh terminal via =multi-term=, with scroll/Yasnippet tweaks so shells stay usable under Evil.
...
Perspectives persist workspaces across restarts; buffer-move and zoom-window cover day-to-day layout tweaks.
...
Ivy/Counsel for completion UI, Projectile-aware file/grep helpers, and Neotree for occasional tree browsing.
...
Projectile for project roots/indexing plus =envrc= so each project can load its own =.envrc=.
...
Flycheck as the primary linter UI; Flymake is available but not forced globally (lsp diagnostics stay disabled so Flycheck owns the gutter).
...
#+BEGIN_SRC emacs-lisp
  (use-package flycheck
    ...
    ;; Org 9.7+ returns propertized string line numbers from `org-lint';
    ;; flycheck still expects a number — coerce before constructing errors.
                            ...
                            (pcase e
                              (`(,_n [,line ,_trust ,desc ,_checker])
                               (flycheck-error-new-at
                                (if (numberp line)
                                    line
                                  (string-to-number (format "%s" line)))
                                nil 'info desc
                                :checker checker))
                              (_
                               (flycheck-error-new-at
...
Structural editing for Lisp-family buffers (and a few others).
...
Color nested delimiters in programming modes (and YAML) for quicker structure scanning.
...
Highlight text past column 100 in selected modes (Clojure/JSON/shell).
...
Global emojify with expensive modes opted out via =emojify-inhibit-major-modes=.
...
#+BEGIN_SRC emacs-lisp
  (use-package which-key
    ;; Built into Emacs 30+; :ensure t still no-ops when the built-in is present.
    :ensure t
...
#+BEGIN_SRC emacs-lisp
  ;;; increase text size while writing, restore on exit
  (defun cs-writeroom-text-scale (arg)
    (text-scale-set (if (= arg 1) 2 0)))

  ;;; hide git diff indicators while writing, restore on exit
  (defvar-local cs-writeroom--diff-hl-was-on nil)
  (defun cs-writeroom-diff-hl (arg)
    (if (= arg 1)
        (when (bound-and-true-p diff-hl-mode)
          (diff-hl-mode -1)
          (setq cs-writeroom--diff-hl-was-on t))
      (when cs-writeroom--diff-hl-was-on
        (diff-hl-mode 1)
        (setq cs-writeroom--diff-hl-was-on nil))))

  (use-package writeroom-mode
    :init
    (setq writeroom-width 80            ; exact, uniform line-length in visible chars
          writeroom-maximize-window nil) ; center in current window only
    :config
    ;; VFC's text-scale margin compensation double-counts the scale factor with
    ;; Emacs 29+ remap widths, making the column window-proportional instead of
    ;; fixed-width; disable it so writeroom-width is the exact line-length.
    (setq visual-fill-column-adjust-for-text-scale nil)
    ;; don't fullscreen the frame on toggle
    (setq writeroom-global-effects
          (remq 'writeroom-set-fullscreen writeroom-global-effects))
    ;; bump text size while writing
    (add-to-list 'writeroom-local-effects #'cs-writeroom-text-scale)
    ;; hide git diff indicators while writing
    (add-to-list 'writeroom-local-effects #'cs-writeroom-diff-hl)
    ;; re-center margins when text size is adjusted interactively
    (advice-add 'text-scale-adjust :after #'visual-fill-column-adjust)
    (evil-leader/set-key "fw" #'writeroom-mode)) ; (f)ocused (w)riting
#+END_SRC
...
:PROPERTIES:
:CUSTOM_ID: language-and-syntax-package-configuration
:END:
...
Tree-sitter grammar sources. Missing grammars are installed automatically on startup (no-op once present).
...
#+BEGIN_SRC emacs-lisp
  (use-package treesit
    ...
    (dolist (lang '(javascript typescript tsx json css html regex ruby python go java))
...
CIDER + clj-kondo + clj-refactor for Clojure/ClojureScript/.cljc editing.

#+BEGIN_SRC emacs-lisp
  (defun cs/cider-common-leader-keys (mode)
    "Shared CIDER Evil leader keys for MODE."
    (evil-leader/set-key-for-mode mode
      ...
      "rtn" #'cider-test-run-ns-tests
      "rtp" #'cider-test-run-project-tests
      "rtl" #'cider-test-run-loaded-tests
      "rtf" #'cider-test-rerun-failed-tests
      "rta" #'cider-auto-test-mode
  ...
  ;; Install before clojure-mode :config may run (cold install dependency ordering).
  ...
  (use-package clojure-mode
    ...
    ;;; keybinds on load
    (cs/cider-common-leader-keys 'clojure-mode)
    (evil-leader/set-key-for-mode 'clojure-mode
      "ri" #'cider-jack-in
      "rc" #'cider-connect)
    (cs/cider-common-leader-keys 'clojurescript-mode)
    (evil-leader/set-key-for-mode 'clojurescript-mode
      "ri" #'cider-jack-in-cljs
      "rc" #'cider-connect-cljs)
    (cs/cider-common-leader-keys 'clojurec-mode))
...
Syntax via =elixir-mode=; language features via ElixirLS through =lsp-mode= (install =elixir-ls= on PATH). Alchemist was removed as unmaintained.
...
Formatting via =goimports=; navigation/completion via =gopls= through =lsp-mode= (shared =,jd= / =,fu= bindings).
...
Tree-sitter JS/TS/TSX modes with project-local ESLint/Prettier, plus =web-mode= for templates.
...
Agenda, capture templates, Mac Mail links, HTML agenda export, and related Org tooling.
...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package org-mode
    ...
    (evil-leader/set-key-for-mode 'org-mode
      ...
      "oP"  'cs-org-preview-file                           ; preview in Org Preview app
...
Elpy remains for now. Prefer migrating to =lsp-pyright= / =python-lsp-server= via =lsp-mode= when Elpy friction shows up.
...
#+BEGIN_SRC emacs-lisp
  ...
  (defconst cs/ruby-tool-manifests '("Gemfile" "gems.rb"))

  (defun cs/ruby-tool-start-directory (path)
    "Return the directory from which Ruby tooling should resolve PATH."
    (let ((expanded (expand-file-name path)))
      (if (file-directory-p expanded)
          (file-name-as-directory expanded)
        (file-name-directory expanded))))

  (defun cs/ruby-tool-search-boundary (directory)
    "Return the Git root or home boundary for DIRECTORY."
    (or (locate-dominating-file directory ".git")
        (let ((home (file-name-as-directory (expand-file-name "~"))))
          (if (string-prefix-p home (file-name-as-directory directory))
              home
            "/"))))

  (defun cs/ruby-tool-ancestor-directories (directory)
    "Return DIRECTORY and its parents through the project boundary."
    (let ((current (file-name-as-directory (expand-file-name directory)))
          (boundary (file-name-as-directory
                     (expand-file-name (cs/ruby-tool-search-boundary directory))))
          directories)
      (while current
        (push current directories)
        (if (string-equal (directory-file-name current)
                          (directory-file-name boundary))
            (setq current nil)
          (let ((parent (file-name-directory (directory-file-name current))))
            (setq current
                  (unless (string-equal (directory-file-name parent)
                                        (directory-file-name current))
                    parent)))))
      (nreverse directories)))

  (defun cs/ruby-tool-nearest-manifest (directories)
    "Return the nearest Bundler manifest under DIRECTORIES."
    (catch 'manifest
      (dolist (directory directories)
        (dolist (name cs/ruby-tool-manifests)
          (let ((path (expand-file-name name directory)))
            (when (file-exists-p path)
              (throw 'manifest path)))))))

  (defun cs/ruby-tool-manifest-declarations (manifest)
    "Return the formatter families declared directly in MANIFEST."
    (let (standardrb rubocop solargraph ruby-lsp reek)
      (when (and manifest (file-readable-p manifest))
        (with-temp-buffer
          (insert-file-contents manifest)
          (goto-char (point-min))
          (while (re-search-forward
                  "^[[:space:]]*gem[[:space:]]*\\(?:([[:space:]]*\\)?[\"']\\([^\"']+\\)[\"']"
                  nil t)
            (let ((name (match-string-no-properties 1)))
              (cond
               ((or (string-equal name "standard")
                    (string-prefix-p "standard-" name))
                (setq standardrb t))
               ((or (string-equal name "rubocop")
                    (string-prefix-p "rubocop-" name))
                (setq rubocop t))
               ((or (string-equal name "solargraph")
                    (string-prefix-p "solargraph-" name))
                (setq solargraph t))
               ((or (string-equal name "ruby-lsp")
                    (string-prefix-p "ruby-lsp-" name))
                (setq ruby-lsp t))
               ((string-equal name "reek")
                (setq reek t)))))))
      (list :standardrb standardrb
            :rubocop rubocop
            :solargraph solargraph
            :ruby-lsp ruby-lsp
            :reek reek)))

  (defun cs/ruby-tool-nearest-file (directories name)
    "Return the nearest file named NAME under DIRECTORIES."
    (catch 'found
      (dolist (directory directories)
        (let ((path (expand-file-name name directory)))
          (when (file-exists-p path)
  ...
  (defun cs/enrich-ruby-tooling (result directories manifest declarations)
    "Add LSP and Reek decisions to formatter RESULT."
    (let ((reek-config (cs/ruby-tool-nearest-file directories ".reek.yml"))
          (solargraph (plist-get declarations :solargraph))
          (ruby-lsp (plist-get declarations :ruby-lsp)))
      (setq result (plist-put result :reek
                              (and (or (plist-get declarations :reek) reek-config) t)))
      (setq result (plist-put result :reek-bundled
                              (and (plist-get declarations :reek) t)))
      (setq result (plist-put result :reek-config reek-config))
      (cond
       ((and solargraph ruby-lsp)
        (setq result (plist-put result :lsp-ambiguous t))
        (setq result (plist-put result :lsp-message
                                (concat "both Solargraph and Ruby LSP are declared in " manifest))))
       (ruby-lsp
        (setq result (plist-put result :language-server 'ruby-lsp-ls))
        (setq result (plist-put result :lsp-bundled t))
        (setq result (plist-put result :lsp-source 'manifest)))
       (solargraph
        (setq result (plist-put result :language-server 'ruby-ls))
        (setq result (plist-put result :lsp-bundled t))
        (setq result (plist-put result :lsp-source 'manifest)))
       (t
        (setq result (plist-put result :language-server 'ruby-ls))
        (setq result (plist-put result :lsp-bundled nil))
        (setq result (plist-put result :lsp-source 'fallback))))
      result))

  (defun cs/resolve-ruby-tooling (&optional path)
    "Resolve project-aware Ruby tooling for PATH or the current buffer."
    (let* ((start (cs/ruby-tool-start-directory
                   (or path buffer-file-name default-directory)))
           (directories (cs/ruby-tool-ancestor-directories start))
           (manifest (cs/ruby-tool-nearest-manifest directories))
           (declarations (cs/ruby-tool-manifest-declarations manifest))
           config-result
           result)
      (catch 'config
        (dolist (directory directories)
          (let ((standard-config (expand-file-name ".standard.yml" directory))
                (rubocop-config (expand-file-name ".rubocop.yml" directory)))
            (cond
             ((and (file-exists-p standard-config)
                   (file-exists-p rubocop-config))
              (setq config-result
                    (list :ambiguous t
                          :message (concat "both .standard.yml and .rubocop.yml exist in " directory)
                          :root directory))
              (throw 'config config-result))
             ((or (file-exists-p standard-config)
                  (file-exists-p rubocop-config))
              (setq config-result
                    (list :tool (if (file-exists-p standard-config) 'standardrb 'rubocop)
                          :bundled (and manifest t)
                          :manifest manifest
                          :root (if manifest (file-name-directory manifest) directory)
                          :source 'config))
              (throw 'config config-result))))))
      (setq result
            (or config-result
                (let ((standardrb (plist-get declarations :standardrb))
                      (rubocop (plist-get declarations :rubocop)))
                  (cond
                   ((and standardrb rubocop)
                    (list :ambiguous t
                          :message (concat "both StandardRB and RuboCop are declared in " manifest)
                          :root (file-name-directory manifest)))
                   ((or standardrb rubocop)
                    (list :tool (if standardrb 'standardrb 'rubocop)
                          :bundled t
                          :manifest manifest
                          :root (file-name-directory manifest)
                          :source 'manifest))
                   (t
                    (list :tool 'rubocop
                          :bundled nil
                          :manifest manifest
                          :root (if manifest (file-name-directory manifest) start)
                          :source 'fallback))))))
      (cs/enrich-ruby-tooling result directories manifest declarations)))

  (defconst cs/project-ruby-exec
    (expand-file-name "~/dotfiles/utils/project-ruby-exec"))

  (defun cs/flycheck-project-command (command)
    "Run Flycheck COMMAND under the project Ruby and bundle when declared."
    (let* ((program (file-name-nondirectory (car command)))
           (bundled
            (cond
             ((member program '("standardrb" "rubocop"))
              (plist-get cs/ruby-tooling-resolution :bundled))
             ((string-equal program "reek")
              (plist-get cs/ruby-tooling-resolution :reek-bundled)))))
      (append (list cs/project-ruby-exec)
              (when bundled '("bundle" "exec"))
              (list program)
              (cdr command))))

  (defun cs/ruby-language-server-command (resolution)
    "Build the project-Ruby LSP command from RESOLUTION."
    (let ((command (list cs/project-ruby-exec)))
      (when (plist-get resolution :lsp-bundled)
        (setq command (append command '("bundle" "exec"))))
      (pcase (plist-get resolution :language-server)
        ('ruby-ls (append command '("solargraph" "stdio")))
        ('ruby-lsp-ls (append command '("ruby-lsp"))))))

  (defun cs/setup-ruby-language-server (resolution)
    "Start only the Ruby language server selected by RESOLUTION."
    (if (plist-get resolution :lsp-ambiguous)
        (progn
          (setq-local lsp-enabled-clients '(ruby-tooling-disabled))
          (message "Ruby LSP disabled: %s" (plist-get resolution :lsp-message)))
      (let ((server (plist-get resolution :language-server))
            (command (cs/ruby-language-server-command resolution)))
        (setq-local lsp-enabled-clients (list server))
        (pcase server
          ('ruby-ls
           (setq-local lsp-solargraph-use-bundler nil
                       lsp-solargraph-server-command command))
          ('ruby-lsp-ls
           (setq-local lsp-ruby-lsp-use-bundler nil
                       lsp-ruby-lsp-server-command command)))
        (lsp-deferred))))

  (defun cs/setup-ruby-tooling ()
    "Select one Ruby formatter and matching Flycheck checker for this buffer."
    (when (fboundp 'chruby-use-corresponding)
      (chruby-use-corresponding))
    (require 'rubocopfmt)
    (let ((resolution (cs/resolve-ruby-tooling)))
      (setq-local cs/ruby-tooling-resolution resolution)
      (setq-local flycheck-disabled-checkers
                  (if (plist-get resolution :reek)
                      (remove 'ruby-reek
                              (and (boundp 'flycheck-disabled-checkers)
                                   flycheck-disabled-checkers))
                    (cl-union (and (boundp 'flycheck-disabled-checkers)
                                   flycheck-disabled-checkers)
                              '(ruby-reek))))
      (if (plist-get resolution :ambiguous)
          (progn
            (when (bound-and-true-p rubocopfmt-mode)
              (rubocopfmt-mode -1))
            (setq-local flycheck-disabled-checkers
                        (cl-union (and (boundp 'flycheck-disabled-checkers)
                                       flycheck-disabled-checkers)
                                  '(ruby-standard ruby-rubocop)))
            (message "Ruby tooling disabled: %s" (plist-get resolution :message)))
        (let ((tool (plist-get resolution :tool))
              (bundled (plist-get resolution :bundled)))
          (setq-local rubocopfmt-rubocop-command (symbol-name tool)
                      rubocopfmt-use-bundler-when-possible bundled
                      flycheck-checker (if (eq tool 'standardrb)
                                           'ruby-standard
                                         'ruby-rubocop)
                      flycheck-command-wrapper-function
                      #'cs/flycheck-project-command)
          (rubocopfmt-mode 1)))
      (cs/setup-ruby-language-server resolution)))

  (add-hook 'ruby-mode-hook #'cs/setup-ruby-tooling)
  (add-hook 'ruby-ts-mode-hook #'cs/setup-ruby-tooling)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (use-package shell-script-mode
    ...
    :defer t
    :mode "\\.sh\\'"
    :hook (shell-script-mode . electric-pair-local-mode)
    :init
...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package scss-mode
    ...
    :mode ("\\.scss\\'"
           "\\.sass\\'")
    :hook (scss-mode . electric-pair-local-mode)
    :config
    (setq css-indent-offset 2))
...
YAML editing with trailing-whitespace cleanup on save.
...
#+BEGIN_SRC emacs-lisp
  ...
  ;; Register after Flycheck loads (:defer t means featurep is false at tangle time)
...
*** preview file with markdown preview

#+BEGIN_SRC emacs-lisp
  (defun cs-md-preview-file ()
    "use the md-preview CLI to preview the current file"
    (interactive)
    (shell-command
     (format "md-preview %s"
...
*** preview file with Org Preview

#+BEGIN_SRC emacs-lisp
  (defun cs-org-preview-file ()
    "preview the current file in Org Preview (org-preview CLI on Linux)"
    (interactive)
    (if (eq system-type 'darwin)
        (start-process "org-preview" nil "open" "-a" "Org Preview" (buffer-file-name))
      (shell-command
       (format "org-preview %s"
               (shell-quote-argument (buffer-file-name))))))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (defun cs/package-json-mentions-p (package-file needle)
    "Return non-nil if PACKAGE-FILE contains NEEDLE (string search)."
    (when (and package-file (file-readable-p package-file))
      (with-temp-buffer
  ...
  (defun cs/use-linter-from-package-json ()
    "Prefer project-local eslint/standard binaries when declared in package.json."
    (let* ((root (or (buffer-file-name) default-directory))
           (package-root (locate-dominating-file root "package.json"))
           (linter-root (locate-dominating-file root "node_modules"))
           ...
           (eslint-p (cs/package-json-mentions-p package-file "eslint"))
           (standard-p (cs/package-json-mentions-p package-file "\"lint\": \"standard")))
      (when package-file
        (cond
         (eslint-p
          (when (and eslint-bin (file-executable-p eslint-bin))
            (setq-local flycheck-disabled-checkers
                        (cl-union flycheck-disabled-checkers
...
#+BEGIN_SRC emacs-lisp
  (defun cs/use-prettier-if-config-exists-in-project-root ()
    "Enable prettier-js-mode when a Prettier config exists in the project."
    (let* ((package-root (locate-dominating-file
                          (or (buffer-file-name) default-directory)
                          "package.json"))
           (package-file (and package-root (expand-file-name "package.json" package-root)))
           (config-names '(".prettierrc"
                           ".prettierrc.json"
                           ".prettierrc.js"
                           ".prettierrc.cjs"
                           ".prettierrc.mjs"
                           ...
                           "prettier.config.cjs"
                           "prettier.config.mjs"))
           (config-file-p (and package-root
                               (cl-some (lambda (name)
                                          (file-exists-p (expand-file-name name package-root)))
                                        config-names)))
           (package-dep-p (cs/package-json-mentions-p package-file "prettier")))
      (when (or config-file-p package-dep-p)
        (prettier-js-mode))))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (when (memq window-system '(mac ns))
    (let ((path-from-shell (string-trim-right
                            (shell-command-to-string "source ~/.profile && printf '%s' \"$PATH\""))))
      (setenv "PATH" path-from-shell)
      (setq exec-path (cl-union (split-string path-from-shell ":") exec-path))))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ;; mac uses Menlo @ 14pt; linux (omarchy) uses omarchy's default font at
  ;; 10pt — same apparent size as mac retina at this panel's pixel density
  (let ((default-font (if (eq system-type 'darwin) "Menlo" "JetBrainsMono Nerd Font"))
        (default-height (if (eq system-type 'darwin) 140 100)))
    (set-face-attribute 'default nil :family default-font :height default-height :weight 'normal))
#+END_SRC
...
:PROPERTIES:
:CUSTOM_ID: general-global-key-bindings
:END:
...
*** attach mode-local evil-leader maps

Buffers already visiting during init (this org file, *scratch*) enabled
evil-leader before =set-key-for-mode= ran.

#+BEGIN_SRC emacs-lisp
  (dolist (buf (buffer-list))
    (with-current-buffer buf
      (cs/reenable-evil-leader-mode)))
#+END_SRC
