...
#+BEGIN_SRC emacs-lisp
  ;;; for company to work for me...
  ;; give me a list but do no completion yet
  ;; tab cycles while completing without changing the list
  ;; doc / help is displayed next to the completion target after a time
  ;; return does not complete and acts as normal return
  (use-package company
    :diminish "→→"
    :config
    (global-company-mode)
